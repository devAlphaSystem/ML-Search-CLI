...
export async function search(query, options = {}) {
  const { limit = DEFAULT_LIMIT, condition, timeout = DEFAULT_TIMEOUT, sort, concurrency: rawConcurrency = DEFAULT_CONCURRENCY, state, category, strict = false, noRateLimit = false, noDetails = false, onFirstResponse = null, onErrorResponse = null } = options;
  const concurrency = noRateLimit ? rawConcurrency : Math.min(rawConcurrency, RATE_LIMIT_CONCURRENCY);
  let _requestCount = 0;
  log("SEARCH", `search("${query}") called`, { limit, condition, sort, state, category, strict, noRateLimit, noDetails, concurrency });
